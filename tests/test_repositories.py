...
from app.db.repositories import create_conversation, create_message, get_conversation, list_conversations, list_messages
...
async def test_conversation_persistence(db_session: AsyncSession) -> None:
    ...
    msg = await create_message(db_session, conv.id, role="user", content="hello", model=None, node_id=None)
    await db_session.commit()
    ...
    msg2 = await create_message(db_session, conv.id, role="assistant", content="hi", model=expected_model, node_id="node1", latency_ms=123)
    await db_session.commit()
    msgs = await list_messages(db_session, conv.id)
